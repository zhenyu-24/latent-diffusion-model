...
class ControlNetModel(pl.LightningModule):
    def __init__(self, num_training_steps=None, vae_path=None, condition_encoder_path=None):
        ...
        checkpoint = torch.load("checkpoints/ldm_ulf/last.ckpt")
        ...
        # 条件encoder
        self.con_encoder = WatEncoder.load_from_checkpoint(condition_encoder_path).eval()
        # VAE模型
    ...
    def configure_optimizers(self):
        # 1. 定义优化器
        optimizer = torch.optim.AdamW(params=list(self.controlnet.parameters()), lr=1e-4, betas=(0.95, 0.999),
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 条件输入
        with torch.no_grad():
            latent_condition = self.con_encoder(input_tensor)
            print("条件输入与潜在空间mse:", F.mse_loss(latent_tensor, latent_condition))
        # 噪声
        ...
        with torch.no_grad():
            ...
            # 损失
            ddpm_loss = F.mse_loss(ddpm_pred, noise)
            print("DDPM Loss:", ddpm_loss.item())
        ...
        if self.global_step % self.sample_steps == 0 and self.global_step > 0:
            with torch.no_grad():
                samples = self.sample(noise, condition=latent_condition, num_inference_steps=self.inference_steps,
                                      cn_cond=input_tensor, mode="concat")
                mse = F.mse_loss(samples, latent_tensor)
                self.log('latent_mse', mse, on_step=True, on_epoch=True, prog_bar=True)
                generated_tensor = self.vae.decode(samples, quantize=False)
                gen_mse = F.mse_loss(generated_tensor, target_tensor)
            ...
            with torch.no_grad():
                vq_tensor = self.vae.decode(latent_tensor, quantize=False)
...
def run():
    ...
    dataloader = patch_train_maskdataloader(dataset, patch_size=(128, 128, 128), batch_size=8, samples_per_volume=8)
    # 加载模型
    num_training_steps = len(dataloader) * 6000
    model = ControlNetModel(num_training_steps=num_training_steps,
                            vae_path='/data/zyxiang/LDM/checkpoints/residual_vqgan/last.ckpt',
                            condition_encoder_path='/data/zyxiang/LDM/checkpoints/ConditionEncoder/last.ckpt')
    print('加载成功')

    trainer = create_ddpmtrainer(name='controlnet', save_dir="./logs", checkpoint_dir="./checkpoints/controlnet",
